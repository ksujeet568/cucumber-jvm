    ...
    private static final class ExclusiveResourceOptions {
        ...
        ExclusiveResourceOptions(ConfigurationParameters parameters, TestTag tag) {
            this.parameters = new PrefixedConfigurationParameters(
                parameters,
                EXECUTION_EXCLUSIVE_RESOURCES_NAME + "." + tag.getName()
            );
        }

        public Stream<String> exclusiveReadWriteResource() {
            return parameters.get(EXECUTION_EXCLUSIVE_RESOURCES_READ_WRITE_NAME, s -> Arrays.stream(s.split(","))
                .map(String::trim))
                .orElse(Stream.empty());
        }

        public Stream<String> exclusiveReadResource() {
            return parameters.get(EXECUTION_EXCLUSIVE_RESOURCES_READ_NAME, s -> Arrays.stream(s.split(","))
                .map(String::trim))
                .orElse(Stream.empty());
        }
